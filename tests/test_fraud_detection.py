...
# Add the parent directory (data-pipeline) to the Python path
...
# Example 1 - Normal Transaction (Should be flagged as safe)
...
# Example 2 - High-Risk Transaction (Should be flagged as fraudulent)
...
# Example 3 - Low-Risk Transaction (Should be flagged as safe)
...
def test_send_fraud_alerts(mock_send_sms, mock_send_slack, mock_send_email):
    ...
    # Assert that all alert functions were called
...
if __name__ == "__main__":
    ...
    test_send_fraud_alerts()  # Run the test for fraud alerts
